...
class ChunkBuffer[T](DataBuffer[T, list[list[T]]]):
    ...
    def __init__(
        ...
        self._current_chunk_queue: deque[T] = deque(maxlen=get_interval)
        self._temporary_queue: deque[list[T]] = deque(
            maxlen=(first_store_steps - first_add_steps) // get_interval + 1
        )
        self._queue: deque[list[T]] = deque(maxlen=max_size)
        self._counter = 0
    ...
    def add(self, data: T) -> None:
        ...
        """
        if self._is_wakeup:
            if self._counter >= self._first_add_steps:
                self._current_chunk_queue.append(data)

            if (
                self._counter + 1 - self._first_add_steps
            ) % self._get_interval == 0 and self._counter >= self._first_add_steps:
                self._temporary_queue.append(list(self._current_chunk_queue))
                self._current_chunk_queue.clear()
            ...
            if (
                self._counter + 1 + self._first_store_steps - self._first_add_steps
    ...
    def save_state(self, path: Path) -> None:
        ...
        with open(path.with_suffix(".pkl"), "wb") as f:
            pickle.dump(
                {
                    "queue": self._queue,
                    "current_chunk_queue": self._current_chunk_queue,
    ...
    def load_state(self, path: Path) -> None:
        ...
        with open(path.with_suffix(".pkl"), "rb") as f:
            ...
            self._current_chunk_queue = deque(
                state["current_chunk_queue"], maxlen=self._get_interval
            )
