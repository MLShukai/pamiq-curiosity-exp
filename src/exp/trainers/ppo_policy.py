...
class PPOLatentPolicyTrainer(TorchTrainer):
    """Trainer for policy using Proximal Policy Optimization (PPO)."""
    ...
    def training_step(self, batch: list[Tensor]) -> dict[str, Tensor]:
        ...
        # Get new distributions and values
        new_dist, new_values, _, _, _ = self.policy_value.model(
            observations, upper_actions, encoder_hiddens[:, 0], predictor_hiddens[:, 0]
        )
        new_log_probs = new_dist.log_prob(actions)
        entropy = new_dist.entropy()
