...
class TestPPOStackedHiddenContinousPiVLatentTrainer:
    DEPTH = 2
    DIM = 8
    OBS_DIM = 16
    OBS_DIM_HIDDEN = 12
    ...
    def trainer(
        ...
        mocker.patch("exp.trainers.ppo_policy.get_global_run")
        return PPOStackedHiddenContinuousPiVLatentTrainer(
            partial_optimizer=partial(AdamW, lr=3e-4),
            gamma=0.99,
            gae_lambda=0.95,
            seq_len=self.SEQ_LEN,
    ...
    def test_run(
        ...
        data_buffers,
        models,
        trainer: PPOStackedHiddenContinuousPiVLatentTrainer,
    ):
        """Test PPO Policy Trainer workflow."""
        models = {
            name: TorchTrainingModel(m, has_inference_model=False, device=device)
    ...
    def test_save_and_load_state(
        self, trainer: PPOStackedHiddenContinuousPiVLatentTrainer, tmp_path: Path
    ):
        """Test saving and loading trainer state."""
        trainer.global_step = 42
        trainer_path = tmp_path / "trainer"
