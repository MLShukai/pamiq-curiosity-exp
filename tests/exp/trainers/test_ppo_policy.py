...
from exp.models.components.fc_scalar_head import FCScalarHead
from exp.models.components.multi_discretes import FCMultiCategoricalHead
from exp.models.components.qlstm import QLSTM
from exp.models.latent_policy import LatentPolicy, ObsUpperActionFlattenHead
...
class TestPPOLatentPolicyTrainer:
    DEPTH = 2
    DIM = 8
    OBS_DIM = 16
    OBS_DIM_HIDDEN = 12
    ...
    def policy_value_model(self):
        ...
        obs_upper_action_flatten_head = ObsUpperActionFlattenHead(
            obs_info=obs_info,
        ...
        action_dist_head = FCMultiCategoricalHead(
            self.DIM,
            self.ACTION_CHOICES,
        )
        value_head = FCScalarHead(
            self.DIM,
        )
        return LatentPolicy(
    ...
    @parametrize_device
    def test_run(self, device, data_buffers, models, trainer: PPOLatentPolicyTrainer):
        """Test PPO Policy Trainer workflow."""
        models = {
            name: TorchTrainingModel(m, has_inference_model=False, device=device)
            for name, m in models.items()
        ...
        for _ in range(20):
            observations = torch.randn(self.OBS_NUM_TOKENS, self.OBS_DIM)
            upper_actions = torch.randn(self.UPPER_ACTION_DIM)
            encoder_hidden = torch.randn(self.DEPTH, self.DIM)
            predictor_hidden = torch.randn(self.DEPTH, self.DIM)
            actions = torch.stack(
                [torch.randint(0, dim, ()) for dim in self.ACTION_CHOICES], dim=-1
            )
            action_log_probs = torch.randn(len(self.ACTION_CHOICES))
            ...
            collector.collect(
                {
                    DataKey.OBSERVATION: observations,
                    DataKey.UPPER_ACTION: upper_actions,
                    DataKey.ENCODER_HIDDEN: encoder_hidden,
                    DataKey.PREDICTOR_HIDDEN: predictor_hidden,
                    DataKey.ACTION: actions,
                    DataKey.ACTION_LOG_PROB: action_log_probs,
                    DataKey.REWARD: rewards,
                    DataKey.VALUE: values,
    ...
    def test_save_and_load_state(self, trainer: PPOLatentPolicyTrainer, tmp_path: Path):
        """Test saving and loading trainer state."""
        trainer.global_step = 42
        trainer_path = tmp_path / "trainer"
        trainer.save_state(trainer_path)
