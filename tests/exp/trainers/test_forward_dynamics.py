...
from exp.trainers.forward_dynamics import (
    ...
    StackedHiddenLatentFDObsInfoTrainer,
    StackedHiddenLatentFDTrainer,
)
...
class TestStackedHiddenLatentFDTrainer:
    BATCH = 4
    DEPTH = 8
    DIM = 16
    DIM_FF_HIDDEN = 32
    ...
    def trainer(
        ...
        mocker.patch("exp.trainers.forward_dynamics.get_global_run")
        trainer = StackedHiddenLatentFDTrainer(
            partial(AdamW, lr=1e-4, weight_decay=0.04),
            seq_len=self.LEN_SEQ,
            max_samples=4,
            batch_size=2,
...
class TestStackedHiddenLatentFDObsInfoTrainer:
    BATCH = 4
    DEPTH = 8
    DIM = 16
    DIM_FF_HIDDEN = 32
    ...
    def trainer(
        ...
        mocker.patch("exp.trainers.forward_dynamics.get_global_run")
        trainer = StackedHiddenLatentFDObsInfoTrainer(
            partial(AdamW, lr=1e-4, weight_decay=0.04),
            seq_len=self.LEN_SEQ,
            max_samples=4,
            batch_size=2,
