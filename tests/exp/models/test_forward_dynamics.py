...
from exp.models.forward_dynamics import StackedHiddenFD, StackedLastHiddenFD
...
class TestStackedLastHiddenFD:
    ...
    @pytest.fixture
    def hidden(self):
        return [torch.randn(self.BATCH_SIZE, self.DIM) for _ in range(self.DEPTH)]

    def test_forward(self, dynamics_model, obs, action, hidden):
        """Test forward pass of StackedLastHiddenFD model."""
        # Run forward pass
        obs_hat_dist, next_hidden = dynamics_model(obs, action, hidden)
        ...
        for h in next_hidden:
            assert h.shape == (self.BATCH_SIZE, self.DIM)
    ...
    def test_single_batch(self, dynamics_model, obs, action, hidden):
        ...
        single_obs = obs[:1]
        single_action = action[:1]
        single_hidden = [h[:1] for h in hidden]
